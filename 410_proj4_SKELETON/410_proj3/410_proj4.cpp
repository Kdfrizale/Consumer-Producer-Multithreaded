...
void test(std::string filename) {
	...
	myThreads.push_back(thread(doWaiter, 0, filename));
	for (int i = 1; i < 20000; i++) {
		myThreads.push_back(thread(doBaker, i));
	}
	auto threadpointer = myThreads.begin();

	while (threadpointer != myThreads.end()) {
		threadpointer->join();
		threadpointer++;
	}
	...
	//int numOfBakers = 300;
//	thread t1(doWaiter, 0, filename);
	//thread threadGroup[300];
	//for (int i = 0; i <  numOfBakers; ++i) {
//		threadGroup[i] = thread(doBaker, i);
//	}
//	t1.join();
//	for (int i = 0; i < numOfBakers; ++i) {
//		threadGroup[i].join();
